...
/**
 * setViewFlow中调用requestLayout();//会导致调用measure()过程 和 layout()从而过程设置最新的页数，修正原来的bug
...
public class CircleFlowIndicator extends View implements FlowIndicator,
    ...
    private static final String TAG = "CircleFlowIndicator";
    private static final int DUFAULT_COUNT = 4;//默认点点点指示器的数目
    ...
    private int count=DUFAULT_COUNT;//生命为全局变量，修正数目不对的bug
...
//            Log.d(TAG,"COUNT->"+count);
    ...
    public void setViewFlow(ViewFlow view) {
        ...
        invalidate();
        requestLayout();//会导致调用measure()过程 和 layout()从而过程设置最新的页数，修正原来的bug
        Log.d(TAG,"setViewFlow,invalidate");
    }

    /*
     * (non-Javadoc)
     *
     * @see FlowIndicator#onScrolled(int, int, int,
     * int)
     */
    @Override
    ...
    private int measureWidth(int measureSpec) {
        Log.d(TAG,"measureWidthCOUNT2->"+count);
        int result = 0;
