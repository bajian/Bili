package com.example.bajian.viewflow;
 ...
 *<p>viewFlow = (ViewFlow)findViewById(R.id.viewflow);
 <p>viewFlow.setAdapter(new ImageAdapter(this));
 <p>viewFlow.setmSideBuffer(3); // 实际图片张数， 我的ImageAdapter实际图片张数为3
 <p>CircleFlowIndicator indic = (CircleFlowIndicator) findViewById(R.id.viewflowindic);
 <p>viewFlow.setFlowIndicator(indic);
 <p>viewFlow.setTimeSpan(4500);
 <p>viewFlow.setSelection(3*1000);	//设置初始位置
 <p>viewFlow.startAutoFlowTimer();  //启动自动播放
  updated 15-8-2 by hgx：getParent().requestDisallowInterceptTouchEvent(true);//阻止父层的View截获touch事件
 */
public class ViewFlow extends AdapterView<Adapter> {

    private static final int DEFAULT_NUM = 4;//默认图片数
    ...
    public ViewFlow(Context context, AttributeSet attrs) {
        ...
        mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, DEFAULT_NUM);
        init();
    }
...
//                return true;
        ...
        return true;//解决scrollView冲突。其他控件可以自行修改
    }
